...
const Weather = ({selectedTab, location, setLocation}) => {
    ...
    useEffect(() => {
       ...
       if(location){
       urlLocation_ =`https://weather.iotstream.ir/getweather/${location?.lat}/${location?.lng}`
       }
       else
       urlLocation_ =`https://weather.iotstream.ir/getweather/35.699739/51.338097`

       let url = urlLocation_
        fetch(url)
    ...
    const handleClickMap = (data) => { //gh برای زمانی که روی نقشه فقط کلیک میکنیم و در کادر جستجو چیزی تایپ و سرچ نمیکنیم
        setCenterMap(data);
    }
    return (
        <>
            <div className="weather bg-white">
                <div className="weather-body">
                    <div className="weather-body-primary">
                        <div className="row">
                            <div
                                className={`col-4 d-flex ${isMobile ? "flex-column justify-content-center align-items-center" : "justify-content-center align-items-center"}`}>
                            ...
                            <div className="col-8 d-flex justify-content-around align-items-center pl-0">
                                <div className="d-flex flex-column justify-content-center align-items-start">
                                    ...
                                    {localStorage.getItem("is_admin") == "true" && <span className="mt-1 cursor-pointer" onClick={() => setIsShowModal(true)}><i
                                        className="icon icon-16 icon-map ml-1" /><span className="font-9 text-nowrap">{t("text.Changelocation")}</span></span>
                                    }
                                </div>
                                <i className={`icon icon-${current?.is_day ? "day" : "night"}-weather-${current?.condition?.code} ${isMobile ? "icon-48" : "icon-48"}`}/>
                            </div>
                        </div>
                    </div>
                                ...
                                className={`d-flex justify-content-between ${isMobile ? "col-6 overflow-x-auto gap-x-12" : "col-8"}`}>
                                {
                                    foreCast?.map((item, index) => {
                                        return (<div className="weather-future" key={index}>
                                            ...
                                            <i className={`icon icon-day-weather-${item.day.condition?.code} ${isMobile ? "icon-16" : "icon-16"}`}/>
                                            <span
                                                className={`${isMobile ? "font-9" : "font-9"}`}>{moment(item.date).locale(`${t("text.langweather")}`).format('dddd')}</span>
                                        </div>)
                                    })
                                }
                            </div>
