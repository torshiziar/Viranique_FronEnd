...
import {
    createActuatorAutomatic, deleteActuatorAutomatic, getActuators, getActuatorsListAutomatic, getSensors, retrieveActuatorAutomatic, updateActuatorAutomatic
} from "../../services/api";
...
const Automatic = ({ automationType, handleShowAutomation }) => {
    ...
    const edit = (item) => {
        if (automationType === 'automatic') {
            ...
            handleShowAutomation({ isOpen: true, type: `${t("text.Automatic")}` })
        }
    };
            ...
            <div
                ...
                <div className="col-lg-6 col-md-12 col-sm-12 px-0">
                    <div className={`${isMobile ? "" : "card p-2"}`}>
                        ...
                        {
                            (actuatorsAutomatic.length === 0 && !isLoading) ? <p className="text-center mt-4 mb-0"> {t("text.Thereisnoinformationdisplay")}</p> : null
                        }
                        {
