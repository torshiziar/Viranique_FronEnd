...
import CO2Temperature from "../Charts/CO2Temperature";
import { useTranslation } from "react-i18next";

const CO2 = ({data, isOnline, value, lastUpdated, interval, fakeValues, graphValue, isConnected}) => {
        ...
        <div className={`d-flex flex-wrap ${data?.size === "small" ? "align-items-center flex-grow-1" : ""}`}>
            <div
                ...
                <div className="d-flex align-items-center">
                    <div className={`icon icon-24 ${isOnline ? "icon-co2" : "icon-co2-off"}`}/>
                    <div className={`h3 mr-2 mb-0 bold ${!isOnline ? "font-24" : "font-30"}`}>
                        ...
                        {
                            isOnline && isConnected && <span className="h6">{data.unit}</span>
                        }
                    </div>
                </div>
                {
            ...
            {(data?.size === "medium" || data?.size === "large") ? value ? (
                <CO2Temperature id={"gauge-" + data.id} value={value[value.length - 1].value}/>) : (<CO2Temperature id={"gauge-" + data.id} unit={data?.unit} value={0}/>) : null
            }
