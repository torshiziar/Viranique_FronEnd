...
import GaugeTSS from "../Charts/GaugeTSS";

const TSS = ({data, isOnline, value, interval, lastUpdated, fakeValues, graphValue, isConnected}) => {
    ...
    return (<div className={`d-flex flex-wrap ${data?.size === "small" ? "align-items-center flex-grow-1" : ""}`}>
        ...
        {(data?.size === "medium" || data?.size === "large") &&
            <GaugeTSS id={"gauge-" + data?.id} value={value ? value[value.length - 1].value : 0} />}
