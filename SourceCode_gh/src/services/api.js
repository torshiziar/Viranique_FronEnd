...
const API_URL = process.env.API_URL || "https://viranique.com/api/";
...
axios.interceptors.request.use( /////////ghafurian for error logout and token
    config => {
        if (localStorage.getItem("token"))
            config.headers['Authorization'] = `Bearer ${localStorage.getItem("token")}`;
        else
            config.headers['Authorization'] = ""; //for login which does not need to token
...
function handleError(error) {
    if (error?.response) {
        ...
        // else if (status === 500) {
        //     alert.show(`${t("text.erroroccurredtheserverpleasetryagain")}`, { type: 'error' });
        // } else if (status === 502) {
        //     alert.show(`${t("text.erroroccurredtheserverpleasetryagain")}`, { type: 'error' });
        // } else if (status === 423 || status === 429) {
        //     alert.show(`${t("text.numberrequestssentwithintimelimitexceededpleasewaitminute")}`, { type: 'error' });
        // } //gh commented
    }
}
...
// export function getGrafana(param){
//     return axios.post('http://mqtt.angizehco.com:8800/dashboard/get_panel/', param)
//     .then(res=>{
//         return res;
//     })
//     .catch(err=>{
...
export function getActuatorsListManual() {
    axios.defaults.headers.common['Authorization'] = "Bearer " + localStorage.getItem("token");///gh
...
export function getremain_account(username) {
    return axios.get(`/accounting/user/${username}/remain_account`)
        .then(res => res.data)
        .catch(err => Promise.reject(err))
}